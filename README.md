...
- `WiFi.h`, `Update.h`, dan `Preferences.h` (Bawaan ESP32 Core)
...
- **`[BASE_TOPIC]/MQTT_OTA/Publisher/start`** : Menerima ukuran file, ukuran chunk, hash MD5 dan firware version dengan format `<file_size><chunk_size><MD5_str><firmware_version>`.
...
- **`[BASE_TOPIC]/MQTT_OTA/Subscriber/OTA_status`** : Topik *feedback* dari ESP ke Broker mengenai status OTA (OK, FAILED, TIMEOUT).

## 🛠️ Cara Penggunaan (Basic Usage)
Lihat folder `examples/Basic_OTA/Basic_OTA.ino` untuk implementasi lengkapnya.
...
1. Inisialisasi Object:
   ...
   #include <MQTT_OTA.h>
   MQTT_Manager MQTT_Client(server, port, username, password, client_id, topic);
   MQTT_OTA MQTT_OTA_Client(MQTT_Client.get_client(), "device/node_1");
   ```
2. Jalankan Inisialisasi Awal:
   ```cpp
   MQTT_OTA_Client.begin();
3. Subscribe ke Topik OTA (otomatis subsribe ke `BASE_TOPIC` jika menggunakan `MQTT_Manager`):
...
5. jalankan fungsi `handle` pada `void loop()` untuk menangani timeout
   ```cpp
